...
my $re_digest_min_flank_3 = 400;
my $slic_5prime_extra = 50;
my %add_sites;
$add_sites{'SapI'} = "GCTCTTC";

my $max_gene_size_slic = 800; 
...
while(my ($gene, $dir) = each(%gene2dir))
{
	...
	my $slic_seq = $re_seq;
	my $ra = Bio::Restriction::Analysis->new(-enzymes=>$rebase_collection, -seq=>$slic_seq);
	...
	{
		$noaddsite{$enz->name} = 1;
		if( !exists($re_avoid{$enz->name})) 
		{
			my $have = "yes";
			if(!exists($re_have{$enz->name}))
			{
				$have = "purchase";
			}

			my $enz_no = 0;
			foreach my $pos ($ra->positions($enz->name))
			{
			...
			foreach my $pos ($ra->positions($enz->name))
			{
				...
				if($prime3_size >= $re_digest_min_flank_3)
				{
					if($prime3_size < $max_cutter_pos && $have eq "yes")
					{
						$max_cutter_pos = $prime3_size;
						$max_cutter_name = $enz->name;
					}
					if($prime3_size < $max_cutter_pos_nohave && $have eq "purchase")
					{
						$nohave_mess .= join("\t", $enz->name, $prime3_size, "purchase") . ";" ;
						$max_cutter_pos_nohave = $prime3_size;
						$max_cutter_name_nohave = $enz->name;
					}
				}
			#	print $prime3_size . ";";
			}
...
#	print "\n";
	...
	if($todo eq "add site")
	{
		...
		# Add SapI site, cuts 16/14 from recognition site
		$slicgene_f = "GCTCTTC" . $slicgene_f;
		$slic_seq_amp_nostop->seq("GCTCTTC" . $slic_seq_amp_nostop->seq());
	}

        my $ra_unq = Bio::Restriction::Analysis->new(-enzymes=>$rebase_collection, -seq=>$slic_seq_amp_nostop);
