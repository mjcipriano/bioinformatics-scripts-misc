...
# insert into phylo_pipe SELECT NULL, orfid, 2, 'Y', sequence from orfs where delete_fg = 'N';
...
my $mbl = Mbl::new(undef, 'giardia09');
my $dbh = $mbl->dbh();
my $fastadir = "/bioware/fasta34/";
my $blastdb_dir = "/";
my $orfdb = "giardia09_orfs_aa";
my $not_include_taxa = "184922";

my $sth = $dbh->prepare("select id as id, idname , sequence, translate from phylo_pipe LIMIT 1");
my $del_pp = $dbh->prepare("delete from phylo_pipe where idname = ?");
...
my $ins_ma = $dbh->prepare("insert into ma (idname, ma, type) VALUES (?, ?, ?)");
my $del_ma = $dbh->prepare("delete from ma where idname = ? and type = ?");
my $ins_tree = $dbh->prepare("insert into tree (idname, tree, type) VALUES (?, ?, ?)");
...
while($keep_going)
{
	...
	if($sth->rows > 0)
	{
		...
		if(scalar @seq_array_aa > 1)
		{
			...
			$ins_ma->execute($orfid, $align_str, 'nexus');
			...
			$ins_tree->execute($orfid, $njtree, 'nj');
