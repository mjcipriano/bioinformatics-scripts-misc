...
if(!defined($scaffold_file) || !defined($contig_file) || !defined($output_prefix))
{
	print "
--scaffold_file
--contig_file
...
sub insert_scaffold
{
	...
	foreach my $contig( @{$scaffold->{contig_array}})
	{
		...
		if($contig->{complement})
		{
			my $delete_h = $dbh->prepare("delete from reads_bases where read_name = ?");
			$delete_h->execute($contig->{contig_accession});
			$insert_reads_bases->execute($contig->{contig_accession}, $seq->seq());
...
#		genbankgff_contig($seq, $contig->{complement});
	}
