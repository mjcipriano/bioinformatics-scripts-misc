#!/usr/bin/perl -w
#
# $Id: load_ncbi_taxonomy.pl,v 1.2 2007/04/16 19:01:15 mcipriano Exp $
#
...
my $gitotaxidtbl = $tablemap{gitotaxid};
...
my %sth = (
	   ...
	   #
           add_gi=>
'INSERT INTO '.$gitotaxidtbl.' (gi, ncbi_taxon_id) VALUES (?, ?)'
           ,
	   add_tax => 
...
#load_gi($dir); M.C. I created a separate script for this, load_gitotaxid.pl
...
sub load_gi
{
	...
	if(-e "$dir/gi_taxid_nucl.dmp")
	{
		open(NUCL, "$dir/gi_taxid_nucl.dmp");
		while(<NUCL>)
		{
			chomp($_);
			my ($gi, $taxid) = split("\t", $_);
			$sth{add_gi}->execute($gi, $taxid);
		}
		open(PROT, "$dir/gi_taxid_prot.dmp");
		while(<PROT>)
		{
			chomp($_);
			my ($gi, $taxid) = split("\t", $_);
...
#    $ftp->get('gi_taxid_nucl.dmp.gz', "$dir/gi_taxid_nucl.dmp.gz");
#    $ftp->get('gi_taxid_prot.dmp.gz', "$dir/gi_taxid_prot.dmp.gz");
    $ftp->quit();

    # unpack them; overwrite previous files, if necessary
    system("gunzip -f $dir/taxdump.tar.gz");
    system("gunzip -f $dir/gi_taxid_nucl.dmp.gz");
    system("gunzip -f $dir/gi_taxid_prot.dmp.gz");
